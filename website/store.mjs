...
export function openStore(filename) {
  ...
  if(!db.prepare('SELECT id FROM state WHERE id=1').get()) {
    const data={schema:1, revision:1, vehicles:structuredClone(vehicles), services:structuredClone(services),
    ...
    for(let day=1;day<=3;day++){const date=new Date(Date.now()+day*86400000).toLocaleDateString('en-CA',{timeZone:'Asia/Taipei'});for(const time of ['10:00','13:30','15:30'])data.slots.push({id:`${date}-${time}`,date,time,status:'OPEN',bookingId:null});}
