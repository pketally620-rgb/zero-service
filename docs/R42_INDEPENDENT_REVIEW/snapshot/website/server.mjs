  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(req.method==='GET'&&path==='/api/catalog'){
        ...
        return send(200,{vehicles:s.vehicles,services:s.services,tierLabels,content:{...s.content,cases:s.content.cases.filter(x=>x.enabled).slice(0,2)},slots:s.slots.filter(x=>x.date>=taipei&&x.status==='OPEN').map(({bookingId,...x})=>x)});
      }
      ...
      if(req.method==='POST'&&path==='/api/bookings'){
        ...
        const b=store.mutate(s=>{requireValue(!s.bookings.some(x=>x.ownerHash===customer.hash&&x.status!=='CANCELLED'&&!x.endedAt),'已有處理中的需求，請先更改或撤回。');const v=s.vehicles.find(x=>x.id===body.vehicleId),service=s.services.find(x=>x.id===body.serviceId);requireValue(v&&service);const slot=s.slots.find(x=>x.id===body.slotId);requireValue(slot&&Date.parse(slot.date+'T'+slot.time+':00+08:00')>now(),'請選擇未來的開放時段。');const bs=bookingStore(s),b=bs.request(body.slotId,{slotDate:slot.date,slotTime:slot.time,ownerHash:customer.hash,vehicleName:v.brand+' '+v.model,serviceName:service.name,quote:priceFor(service,v),createdAt:new Date(now()).toISOString(),pendingSince:new Date(now()).toISOString()});saveBookings(s,bs);return publicBooking(b);});return send(201,b);
      }
