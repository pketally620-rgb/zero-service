...
test('controlled candidate: auth, isolation, persistence, operations and recovery',async()=>{
 ...
 try{
  ...
  const v=catalog.vehicles[0];assert.equal((await request(owner,'/api/manage/vehicle',{...v,override:'special'})).status,200);assert.equal((await request(a,'/api/quote',{vehicleId:v.id,serviceId:'wash'})).data.amount,2500);
  ...
  const publicContent=(await request(a,'/api/catalog')).data.content;assert.equal(publicContent.cases.length,2);assert.equal(publicContent.cases[0].id,'third');assert.equal(publicContent.featured.enabled,false);
