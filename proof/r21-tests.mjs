...
test('R2.1 wording follows actual request/confirm/change/cancel transitions without exposing raw states',()=>{
 ...
 assert.equal(check().lineAction,'前往 LINE 完成確認');assert.equal(check().change,'更改申請時段');assert.equal(check().cancel,'撤回預約需求');
 store.confirm(b.id);assert.equal(check().lineAction,'前往 LINE 聯繫 ZERO');assert.equal(check().status,'預約已確認');assert.equal(check().cancel,'申請取消預約');
 ...
 store.cancel(b.id);assert.equal(check().status,'預約需求已撤回');assert.equal(check().cancel,null);assert.equal(check().lineAction,null);
 const c=store.request('S1',{});store.confirm(c.id);store.cancel(c.id);assert.equal(bookingWording(c).status,'預約已取消');assert.equal(bookingWording(c).lineAction,null);
