import { brandsFor, modelsFor, lineHandoffUrl, LINE_PROFILE_URL } from './ux.mjs';
const $=s=>document.querySelector(s);
const money=n=>new Intl.NumberFormat('zh-TW',{style:'currency',currency:'TWD',maximumFractionDigits:0}).format(n);
const stateNames={OPEN:'可申請',PENDING:'待人員確認',CONFIRMED:'已確認',CANCELLED:'已取消',CLOSED:'暫停開放'};
...
function action(fn){return async e=>{e?.preventDefault();try{await fn(e);}catch(error){toast(error.message);}};}
...
async function renderPrice(){
  ...
  $('#rule').textContent='自動分類：'+catalog.tierLabels[q.autoTier]+(catalog.vehicles.find(x=>x.id===vehicleId).ownerOverrideTier?' → Owner 指定：'+catalog.tierLabels[q.tier]:'');
}

function renderSlots(){$('#vehicleRequired').hidden=!!$('#vehicle').value;const box=$('#slots');box.replaceChildren();for(const slot of catalog.slots){const b=document.createElement('button');b.className='slot';b.disabled=slot.status!=='OPEN'||!$('#vehicle').value;const title=document.createElement('b');title.textContent=`${slot.date.slice(5).replace('-','/')} ${slot.time}`;const status=document.createElement('small');status.textContent=stateNames[slot.status];b.append(title,status);b.onclick=action(async()=>{if(!$('#vehicle').value)throw Error('請先選擇品牌與車款。');if(currentBooking && currentBooking.status!=='CANCELLED')throw Error('已有預約需求，請使用改期或取消。');currentBooking=await api('/api/bookings',{slotId:slot.id,vehicleId:$('#vehicle').value,serviceId:$('#service').value});await refresh();renderBooking();});box.append(b);}}
function summaryText(b){const slot=catalog.slots.find(s=>s.id===b.slotId);return `【ZERO Proof 合成預約需求・請勿正式送出】\n編號：${b.id}\n服務：${b.serviceName}\n車款：${b.vehicleName}\n分類：${catalog.tierLabels[b.quote.tier]}\n送出時基準價：${b.quote.type==='amount'?money(b.quote.amount):'現場評估'}\n時段：${slot.date} ${slot.time}\n狀態：${stateNames[b.status]} (${b.status})\n網站申請不代表正式預約，仍需人員確認。`;}
function renderBooking(){const b=currentBooking,box=$('#summary');box.innerHTML='<h3>你的預約摘要</h3>';const status=document.createElement('span');status.className='status';status.textContent=stateNames[b.status];const ta=document.createElement('textarea');ta.rows=9;ta.readOnly=true;ta.setAttribute('aria-label','LINE 預約摘要');ta.value=summaryText(b);const history=document.createElement('p');history.className='small muted';history.textContent=b.history.join(' → ');box.append(status,history,ta);const note=document.createElement('p');note.className='small muted';note.textContent=b.status==='CANCELLED'?'此預約已取消。':'金額保留送出時的基準價。前往 LINE 後，確認預填內容再送出，由人員完成預約確認。';box.append(note);
if(b.status!=='CANCELLED'){
  ...
  const guide=document.createElement('p');guide.className='small muted';guide.textContent='請使用已安裝 LINE 的手機。Proof 請勿實際送出。';box.append(guide);
  const fallback=document.createElement('details');const heading=document.createElement('summary');heading.textContent='電腦版或未帶入內容？';fallback.append(heading);
  const help=document.createElement('p');help.className='small muted';help.textContent='電腦版 LINE 不支援此預填方式。若未帶入內容，可使用下方備援：一次複製並開啟官方 LINE，再貼上、確認後送出。';fallback.append(help);
  ...
  copy.onclick=async()=>{
    try{
      ...
      feedback.textContent='預約內容已複製。請在 ZERO 官方 LINE 對話框貼上，確認後送出。Proof 請勿實際送出。';
      if(!next){const reopen=document.createElement('a');reopen.className='btn';reopen.href=LINE_PROFILE_URL;reopen.target='_blank';reopen.rel='noopener noreferrer';reopen.textContent='瀏覽器擋住新視窗：開啟 LINE';feedback.append(reopen);}
    }catch(error){feedback.textContent='瀏覽器未允許複製；內容尚未複製。請改用手機的「前往 LINE 完成確認」。';}
  };
  fallback.append(copy,feedback);box.append(fallback);
}
if(b.status==='CANCELLED')return;const label=document.createElement('label');label.textContent='改期至';const changeSelect=document.createElement('select');changeSelect.id='changeSlot';options(changeSelect,catalog.slots.filter(s=>s.status==='OPEN'),s=>`${s.date} ${s.time}`);label.append(changeSelect);box.append(label);const change=document.createElement('button');change.className='btn';change.textContent='申請改期';change.disabled=!changeSelect.options.length;change.onclick=action(async()=>{currentBooking=await api(`/api/bookings/${b.id}/change`,{slotId:changeSelect.value,capability:b.capability});await refresh();renderBooking();});const cancel=document.createElement('button');cancel.className='btn';cancel.textContent='取消這筆需求';cancel.onclick=action(async()=>{currentBooking=await api(`/api/bookings/${b.id}/cancel`,{capability:b.capability});await refresh();renderBooking();});const end=document.createElement('div');end.className='actions';end.append(change,cancel);box.append(end);}
